...
**Déclaration (`mobile_money_transactions`, `channel = DECLARED`, statut `DECLARED`).** Aucun paiement n'est créé à la déclaration : le `payment` naît à la validation (voir ci-dessous). La contrainte `UNIQUE (provider, provider_transaction_id)` empêche qu'une même référence opérateur serve deux fois.

**Validation.** Deux chemins, au choix de l'organisation : validation manuelle par un ACCOUNTANT ou un MANAGER (`payment:validate_transfer`, même permission que le virement déclaré), ou rapprochement avec un relevé d'opérateur importé lorsque l'organisation en dispose (relevé agent MTN/Airtel, même mécanique qu'au §8.3). La validation crée un paiement `CONFIRMED`, sans commission, ses allocations sont écrites et la quittance est générée. Le rejet exige un motif (`REJECTED` + `rejection_reason`), notifié au locataire, et ne laisse aucune ligne dans `payments`.
...
**Déclaration (`bank_transfer_declarations`).** Le locataire déclare son virement depuis le portail : montant, date d'exécution, banque émettrice, référence utilisée, et **preuve** (photo du bordereau ou capture d'écran, stockée sur R2 via le module `documents`). Aucun paiement n'est créé à la déclaration : le `payment` naît à la validation (voir ci-dessous). La même preuve ne peut servir deux fois dans l'organisation : contrôle applicatif sur l'empreinte SHA-256 du document (`documents.checksum_sha256`), erreur `BANK.PROOF_ALREADY_USED`, ce qui bloque la fraude la plus simple (rejouer un ancien bordereau).

**Validation.** Un ACCOUNTANT ou un MANAGER valide (`payment:validate_transfer`) selon la politique `paymentMethods.bankTransfer.confirmOnApproval` de l'organisation (défaut `true`) : si `true`, création du paiement `CONFIRMED`, alloué, avec quittance ; si `false`, création du paiement `PENDING_VERIFICATION` non alloué, facture affiche « paiement déclaré, en attente de confirmation bancaire » jusqu'au rapprochement de la phase 6. Le rejet exige un motif (`REJECTED` + `rejection_reason`), notifié au locataire, et ne laisse aucune ligne dans `payments`. Un délai de validation supérieur à 72 h ouvrées déclenche une alerte : le locataire a payé, il attend sa quittance.
