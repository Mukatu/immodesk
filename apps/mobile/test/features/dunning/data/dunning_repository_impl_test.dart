...
void main() {
  ...
  test('transmet le filtre par locataire et trie du plus récent au plus ancien '
      '(dates ancrées)', () async {
    when(
      () => remote.fetchRuns(
        'org-1',
  ...
  test('ouverture depuis une facture filtre par invoiceId sans transmettre '
      'tenantId', () async {
    when(
      () => remote.fetchRuns(
        'org-1',
    ...
    expect(result.data.map((r) => r.id).toList(), ['run-invoice']);
    verify(
      () => remote.fetchRuns(
        'org-1',
        invoiceId: 'invoice-1',
