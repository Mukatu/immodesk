...
**Encaissements — 4 modes.** Enregistrement des règlements en espèces (avec reçu numéroté et signature), en Mobile Money (initiation et confirmation par re-interrogation du statut auprès de l'agrégateur), par virement bancaire déclaré (téléversement de preuve par le locataire puis validation), et en version simplifiée par chèque (saisie du numéro et de la banque, sans automatisation de la compensation en version 1). Un paiement peut être partiel et s'imputer sur une ou plusieurs factures.
...
- Stockage fichiers : Cloudflare R2, URLs signées.
- Génération PDF : Puppeteer dans un worker dédié.
- WhatsApp : Meta WhatsApp Cloud API (templates approuvés), SMS de secours via interface `SmsProvider`.
- Mobile Money : interface `MobileMoneyProvider`, première implémentation via agrégateur (CinetPay), avec confirmation systématique par re-interrogation du statut côté agrégateur.
- Authentification par téléphone + OTP, contrat d'API OpenAPI 3.1.
- Infrastructure Docker, déploiement région Europe (Paris), CI/CD GitHub Actions, supervision Sentry + Grafana/Prometheus, sauvegardes PostgreSQL quotidiennes chiffrées.
...
- **Exigences BEAC/COBAC** : si l'agrégateur ou Immodesk venait à détenir des fonds pour le compte de tiers (au-delà du simple rôle d'intermédiaire technique de paiement), un agrément ou un partenariat avec un établissement agréé serait requis. Le principe retenu pour la version 1 est qu'**Immodesk ne détient jamais les fonds** : les flux Mobile Money transitent par un agrégateur agréé, les espèces et virements restent gérés directement entre le locataire, le démarcheur/agence et le bailleur.
...
- Coût des SMS et de la bande passante WhatsApp : privilégier WhatsApp (moins coûteux à l'usage une fois le compte vérifié) avec repli SMS uniquement en cas d'échec, et maîtriser le volume de notifications.
...
- La Phase 4 (Mobile Money) dépend directement de la signature du contrat avec l'agrégateur (hypothèse H1, section 9) : un retard sur cette dépendance externe décale d'autant la Phase 4 sans nécessairement décaler les phases suivantes si l'équipe peut avancer sur la Phase 5 en parallèle.
