...
/**
 * Détecte la colonne d'actions (pied de carte en vue mobile) : `id: 'actions'` couvre les
 * trois formes d'en-tête réellement présentes dans le code (chaîne vide, chaîne « Actions »,
 * ou fonction) puisque chacune de ces colonnes porte cet id ; les deux vérifications de
 * chaîne restent en filet de sécurité pour une éventuelle colonne d'actions sans cet id.
 */
function isFooterColumn(column: { id: string; columnDef: { header?: unknown } }): boolean {
  if (column.id === 'actions') return true;
  const header = column.columnDef.header;
  return typeof header === 'string' && (header === '' || header === 'Actions');
}

/**
 * Colonne de sélection (case à cocher de tête de ligne). Elle ne peut pas servir de titre
 * de carte — ce serait une case à cocher mise en évidence à la place du numéro de facture —
 * et elle n'a pas non plus sa place dans les paires étiquette/valeur : elle est rendue comme
 * un contrôle en tête de carte, à gauche du titre.
 */
function isSelectionColumn(column: { id: string }): boolean {
...
export function DataTable<TData, TValue>({
  ...
  // Vue carte (< 640px) : en-têtes indexés par id de colonne, pour retrouver l'en-tête d'une
  // cellule sans relire de chaîne (flexRender exige le couple header + son propre contexte).
  //
  // NE PAS transformer en `useMemo` : un hook s'exécuterait AVANT les deux sorties anticipées
  // ci-dessus, donc `table.getHeaderGroups()` serait appelé même sans données. C'est ce qui a
  // fait échouer le prérendu de /app/paiements/mobile-money, dont une colonne a un en-tête
  // vide sans identifiant. Ici, le calcul n'a lieu que lorsqu'il y a des lignes à rendre —
  // exactement comme avant l'ajout de la vue carte. Le coût est négligeable (une page de
  // résultats).
  ...
  return (
    <div className="space-y-3">
      {/* Vue tableau : inchangée, simplement masquée sous 640px. */}
      ...
      {/* Vue carte : masquée dès 640px. Construite à partir des mêmes définitions de colonnes,
          donc aucun écran appelant n'a besoin d'être modifié pour en bénéficier. */}
      <div className="space-y-3 sm:hidden">
        {table.getRowModel().rows.map((row) => {
          const visibleCells = row.getVisibleCells();
          // La case à cocher de sélection est extraite avant le choix du titre : sans cela,
          // c'est elle qui deviendrait le titre de la carte (cas de l'écran des factures).
          const selectionCell = visibleCells.find((cell) => isSelectionColumn(cell.column));
          const [titleCell, ...otherCells] = visibleCells.filter((cell) => cell !== selectionCell);
          const footerCell = otherCells.find((cell) => isFooterColumn(cell.column));
          const middleCells = otherCells.filter((cell) => cell !== footerCell);

          return (
            <Card
              key={row.id}
              className={cn(getRowClassName?.(row.original), onRowSelect && 'cursor-pointer')}
              onClick={
                onRowSelect
                  ? (event) => {
                      const target = event.target as HTMLElement;
            ...
            >
              <CardHeader className="flex-row items-center justify-between gap-3 space-y-0 p-4">
                ...
                {onRowSelect ? (
                  // Vrai bouton natif (voir le commentaire au-dessus d'onRowSelect) : toujours
                  // visible ici (pas d'apparition au survol, absent au doigt), et de 44px au
                  // moins de côté pour rester atteignable au toucher.
                  <button
                    type="button"
                    onClick={() => onRowSelect(row.original)}
                    aria-label={getRowLabel?.(row.original) ?? 'Voir le détail de cette ligne'}
                    className="flex size-11 shrink-0 items-center justify-center rounded-sm text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
              ...
              {middleCells.length > 0 ? (
                <CardContent className="space-y-2 p-4 pt-0 text-sm">
                  {middleCells.map((cell) => {
                    ...
                    const hasLabel = !(typeof headerDef === 'string' && headerDef === '');
                    return (
              ...
              {footerCell ? (
                <CardFooter className="p-4 pt-0">
                  <div className="w-full">
                    {flexRender(footerCell.column.columnDef.cell, footerCell.getContext())}
                  </div>
                </CardFooter>
              ) : null}
            </Card>
